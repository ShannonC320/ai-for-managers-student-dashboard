...
import { formatHours, localDate, priorities, readTasks, summarize, taskStatus, validateTask, writeTasks } from './planner.js';

const blankTask = () => ({ title: '', category: '', dueDate: '', hours: '', priority: 'Medium', notes: '', completed: false });
...
export default function Planner() {
  ...
  function commit(next, success) {
    try {
      writeTasks(next);
      setTasks(next); setError(''); setMessage(success);
      return true;
    } catch {
  ...
  return (
    <main className="page planner-page" id="main-content" tabIndex="-1">
      <div className="page-heading">
        ...
        <button ref={addButton} className="primary-button" disabled={!!initial.error || !!draft} onClick={() => { setDraft(blankTask()); setError(''); setMessage(''); setDeleting(null); }}>+ Add task</button>
      </div>
          ...
          {!visible.length ? <div className="empty-state"><h3>{tasks.length ? 'No tasks in this view' : 'A clear place to start'}</h3><p>{tasks.length ? 'Choose another view to see your tasks.' : 'Add an assignment, estimate the effort, and choose its priority. Your plan starts here.'}</p></div> :
            <ul className="task-list">{visible.map(task => {
              ...
              return <li key={task.id} className={`task-item ${task.completed ? 'is-complete' : ''}`}>
                ...
                {deleting === task.id && <div className="delete-confirmation"><p>Delete “{task.title}”? This cannot be undone.</p><button className="remove-button" onClick={() => { if (commit(tasks.filter(item => item.id !== task.id), 'Task deleted.')) { setDeleting(null); addButton.current?.focus(); } }}>Confirm delete</button><button className="secondary-button" onClick={() => setDeleting(null)}>Keep task</button></div>}
              </li>;
            })}</ul>}
        </section>

        <aside className="planning-sidebar" aria-label="Planning overview">
          <section className={`panel workload-panel ${summary.heavy ? 'heavy-workload' : ''}`}>
            <div className="eyebrow">Today & tomorrow</div><h2>{formatHours(summary.hours)} {summary.hours === 1 ? 'hour' : 'hours'} · {summary.nearCount} {summary.nearCount === 1 ? 'task' : 'tasks'}</h2>
            <p className="workload-verdict">{summary.heavy ? 'Potentially heavy workload' : 'Below the workload flag threshold'}</p>
            <p className="muted">A review flag appears at 8+ estimated hours or 3+ unfinished deadlines across these two calendar dates. This is a rule, not a judgment of your capacity.</p>
            {summary.overdue > 0 && <p className="danger-text">Also review {summary.overdue} overdue {summary.overdue === 1 ? 'task' : 'tasks'} ({formatHours(summary.overdueHours)} hrs), not included above.</p>}
          </section>
          <section className="panel" aria-labelledby="week-workload"><h2 id="week-workload">Next 7 days</h2><p className="muted">Estimated hours grouped by due date, not scheduled work sessions.</p>
            <ul className="workload-days">{summary.days.map(day => <li key={day.date}>
              ...
              <div className={`workload-track ${day.heavy ? 'flagged' : ''}`} aria-hidden="true"><span style={{ width: `${Math.min(100, day.hours / 8 * 100)}%` }} /></div>
              {day.heavy && <small className="danger-text">Review this day: 8+ hours or 3+ deadlines</small>}
            </li>)}</ul>
          </section>
          <section className="panel planning-support"><h2>Planning Support</h2><p>Tools can organize deadlines and surface possible conflicts. This planner uses simple rules, with no AI service.</p><p>Start with urgent work, compare effort and importance, then revise your plan. You choose every priority; sorting never changes it.</p><p>Like a manager reviewing a 48-hour plan, consider your available time and commitments before deciding what comes first.</p></section>
        </aside>
      </div>
