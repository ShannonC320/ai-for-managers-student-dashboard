...
export function DependencyPicker({ tasks, value = [], onChange, currentId }) {
  const options = tasks.filter(task => task.id !== currentId && !task.completed);
  ...
  return (
    <fieldset className="dependency-picker">
      ...
      <div className="checklist">
        ...
        {missing.map(id => (
          <label className="danger-text" key={id}>
            <input
              type="checkbox"
              checked
              onChange={() => toggle(id)}
...
export function PromptOutput({ value, label }) {
  ...
  async function copy() {
    try {
      ...
      setMessage('Prompt copied. Paste it into your approved AI tool.');
    } catch {
      setMessage(
        'Copy was unavailable. Select the prompt text below and copy it manually.'
      );
    }
  }
